...
export const styles = StyleSheet.create({
    ...
    formDescription: {marginBottom: 32, color: '#2196F3', fontWeight: 'bold', opacity: 0.69},
...
class FormItem extends React.PureComponent<{ setValue(x: string): void, label: string, icon: string, tintColor: string, password?: boolean }, { inputValue: string }> {
        ...
        return (
            <View style={styles.formItem}>
                <View style={styles.inputIcon}>
                    <Ionicons name={this.props.icon} size={25} color={this.props.tintColor}/>
                </View>
...
export class RequireAuthComponent extends React.Component<RequireAuthProps, { showSignUp: boolean, isProgressing: boolean }> {
    ...
    signUpForm() {
        return <View>
            <View style={styles.form}>
                ...
                <FormItem label={'Password'} setValue={((x) => {
                    this.password = x
                })} icon={'ios-key-outline'} tintColor={'#90CAF9'}
                          password={true}/>
                <Divider style={{margin: 16}}/>
                <FormItem label={'Username'} setValue={((x) => {
                    this.username = x
