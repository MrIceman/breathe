...
export class HomeComponent extends React.Component<{}, HomeComponentState> {
    ...
    private getLastPostCard() {
        ...
        return <Card title={'Last Post'}>
            <View style={this.styles.lastPostCard}>
                <View style={this.styles.lastPostAuthor}>
                    <Text style={{fontWeight: 'bold', color: '#1976D2'}}>Mr Iceman</Text>
                </View>
                <View style={this.styles.lastPostImage}>
                    <Image style={{width: 150, height: 80}}
                           source={{uri: 'http://www.abc.net.au/news/image/7515684-3x2-940x627.jpg'}}/>
                </View>
            </View>
        </Card>
    ...
    private getWelcomeCard() {
        return <Card title={`Welcome ${this.state.currentUsername}`} containerStyle={this.styles.welcomeCard}>
            <View style={this.styles.cardContentView}>
                <Text>{this.state.welcomeText}</Text>
                <TouchableOpacity>
                    <Text style={this.styles.welcomeCardActionButton}>Start a Session</Text>
                </TouchableOpacity>
            </View>
        </Card>;
    }
    ...
    private getStatisticsCard() {
        return <Card title={'Quick Stats'}>
            <View style={this.styles.statisticsCard}>
                <Text>This Week you have done <Text style={{fontWeight: 'bold'}}>0 Breathing Sessions</Text>.</Text>
                <Text>You meditated for <Text style={{fontWeight: 'bold'}}>0 Minutes</Text> in total</Text>
                <Text>and you were out for <Text style={{fontWeight: 'bold'}}>0 Minutes</Text> in the cold.</Text>
            </View>
        </Card>;
    }
